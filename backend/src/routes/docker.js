...
async function statsSample(server) {
  await check();
  if (mock || !client) return null;
  const c = await getContainer(server);
  if (!c) return null;
  ...
  const cpus =
    Number(s?.cpu_stats?.online_cpus || 0) ||
  ...
  return {
    ...
    memUsed: Number(s?.memory_stats?.usage || 0),
    mock: false,
  };
}
